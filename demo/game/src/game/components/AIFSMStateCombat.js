/**
* Game custom FSM AI Combat - extends FSMStateComposite
*/
var AIFSMStateCombat = Lucid.FSMStateComposite.extend({
	...
	init: function(config) {
		...
		if (!this.ai) {
			Lucid.Utils.error("AIFSMStateCombat @ init: ai is null!");
			return false;
		}
			...
			if (!collisionData) {
				...
				if (targetEntity.type == Lucid.BaseEntity.TYPE.UNIT && targetEntity.team != originEntity.team) {
					...
					if (Lucid.Math.getDistanceBetweenTwoEntities(targetEntity, originEntity) > targetEntity.minimumAttackRange) {
						this.fsm.eventName = AIFSMFighter.EVENTS.ENEMY_NOT_IN_RANGE;
					}
